...
function Cal() {
  ...
  if (R1 > 5 && R2 > 5 && R3 > 5 && R1 != R2 && R2 != R3 && R1 != R3 && T1 != T2) {
    ...
    C =
      (1 / T1 -
        1 / T2 -
        ((Math.log(R1) - Math.log(R2)) * (1 / T1 - 1 / T3)) / (Math.log(R1) - Math.log(R3))) /
      (Math.pow(Math.log(R1), 3) -
        Math.pow(Math.log(R2), 3) -
        ((Math.log(R1) - Math.log(R2)) * (Math.pow(Math.log(R1), 3) - Math.pow(Math.log(R3), 3))) /
          (Math.log(R1) - Math.log(R3)))
    B =
      (1 / T1 - 1 / T2 - C * (Math.pow(Math.log(R1), 3) - Math.pow(Math.log(R2), 3))) /
      (Math.log(R1) - Math.log(R2))
...
function draw() {
  if (Fail < 1) {
    ...
    while (Rt < Rtm) {
      ...
      //document.write(Rt +'_');
      SHH.push([
        1 / (A + B * Math.log(Rt) + C * Math.log(Rt) * Math.log(Rt) * Math.log(Rt)) - 273.15,
        Rt,
      ]) //1/(A+B*Math.log(Rt)+C*(Math.log(Rt))*(Math.log(Rt))*(Math.log(Rt)))-273.15
